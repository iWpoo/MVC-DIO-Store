...
class CartService
{
    ...
    {
        $cart = (new Cart)->findSpecific("WHERE user_id = :user_id AND product_id = :product_id", [':user_id' => $user_id, ':product_id' => $product_id]);
        if (!$cart) {
            (new Cart)->create([
                'user_id' => $user_id,
    ...
    // Проверка на существование куки cart
    ...
    // Удаление или добавления товаров в корзинку (куки)
